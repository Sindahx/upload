...
if __name__ == '__main__':
    ...
    userInfo = bot.search_friends(name='Loveyou1234567890')
    print(userInfo)
    bot.send("啦啦啦啦啦啦啦啦啦啦", toUserName=userInfo[0]["UserName"])
...
def get_whole_msg(msg, download=True):
    sender, receiver = get_sender_receiver(msg) #获取收信人和发信人
...
@bot.msg_register([TEXT, PICTURE, MAP, CARD, SHARING, RECORDING,ATTACHMENT, VIDEO, FRIENDS], isFriendChat=True, isGroupChat=True)
def normal_msg(msg):
    # print(msg['Text'])
    msg_content = get_whole_msg(msg)
    if(msg['Type'] == "Picture"):
        if(len(msg_content)):
            fn = os.path.join(data_path, msg['FileName'])
            bot.send_image(fn, toUserName=userInfo[0]["UserName"])
    else:
        bot.send(' '.join(msg_content), toUserName=userInfo[0]["UserName"])

    print_msg(msg_content)
    now = time.time()
